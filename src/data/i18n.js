export const translations = {
  es: {
    nav: {
      about: 'Sobre mí',
      projects: 'Proyectos',
      education: 'Formación',
      skills: 'Skills',
      activity: 'Actividad',
      contact: 'Contacto',
      openMenu: 'Abrir menú',
    },
    availability: {
      available_remote: 'Disponible',
      available_onsite: 'Disponible',
      not_available: 'No disponible',
    },
    hero: {
      title: 'Fullstack & AI Developer',
      description: 'Construyo SaaS y herramientas de IA — del prototipo a producción.',
      cta: 'Ver proyectos',
      aboutLabel: '// sobre mí',
      terminal: {
        hint: "escribe 'help' para explorar",
        whoami: ['Deiby Gorrin', 'Fullstack Developer · disponible para oportunidades'],
        help: [
          'help          → muestra este mensaje',
          'ls            → lista las secciones',
          'goto <sección>→ navega a una sección',
          'open github   → abre GitHub',
          'open linkedin → abre LinkedIn',
          'whoami        → sobre mí',
          'clear         → limpia la terminal',
        ],
        ls: ['hero', 'about', 'projects', 'education', 'skills', 'activity', 'contact'],
        notFound: (cmd) => `comando no encontrado: ${cmd}. Escribe 'help'.`,
        goto: (section) => `→ navegando a #${section}`,
        gotoUnknown: (section) => `sección desconocida: ${section}. Usa 'ls' para ver las disponibles.`,
        opening: (target) => `→ abriendo ${target}...`,
      },
      bio1: 'Publico y mantengo herramientas open source para agentes de IA con uso real y medible: ClaudeStat (npm) da trazabilidad de costos y detección de loops para Claude Code y OpenCode — más de 2.500 sesiones trackeadas en 22 proyectos — y applyr (PyPI), una CLI nativa para agentes que puntúa ofertas de trabajo y genera CVs ATS-safe; la usé para analizar más de 300 ofertas reales.',
      bio2: 'Desarrollador Fullstack con DAM y Máster Full Stack finalizados, especialización en ecosistemas Oracle. Experiencia práctica como Junior Developer en Mercanza, donde construí módulos de un ERP modular con Kafka, FastAPI y Docker. En paralelo, construyo WodRival — plataforma de fitness competitivo con temática vikinga, en Next.js y Supabase.',
    },
    projects: {
      label: '// proyectos',
      title: 'Lo que he construido',
      github: 'Ver en GitHub',
      live: 'Ver demo',
    },
    skills: {
      label: '// tecnologías',
      title: 'Stack tecnológico',
      packagesLoaded: (n) => `${n} paquetes cargados`,
      levelLabels: { advanced: 'avanzado', intermediate: 'intermedio', basic: 'básico', learning: 'aprendiendo' },
      sourceLabels: { project: 'proyecto', practice: 'práctica', cert: 'certificación' },
    },
    education: {
      label: '// formación',
      title: 'Formación & Certificaciones',
      certTitle: 'Certificaciones',
      inProgress: 'En curso',
      coursesLoaded: (n) => `${n} cursos cargados`,
      certsVerified: (n) => `${n} certificados verificados`,
      verified: '✓ verificado',
      viewPdf: 'ver PDF',
    },
    certModal: {
      download: 'descargar',
      closeHint: 'ESC para cerrar · click fuera para cerrar',
      openNewTab: 'abrir en nueva pestaña ↗',
    },
    about: {
      label: '// sobre mí',
      terminalFile: 'profile.sh',
      profileCmd: 'cat profile.sh',
      initLine: '> cargando perfil...',
      nameLine: '> nombre: "Deiby Gorrin"',
      roleLine: '> rol: "Fullstack Developer | AI Developer Tooling"',
      skillsFile: 'skills.sh',
      skillsCmd: 'verify-skills --all',
      status: 'disponible · abierto a oportunidades',
      checksPassed: (n) => `${n} aprobados · 0 fallos`,
      groups: [
        {
          title: 'Perfil profesional',
          items: [
            'Pensamiento analítico',
            'Orientación a resultados',
            'Disponibilidad inmediata',
            'Formación completada (DAM + Máster Full Stack)',
            'Adaptación rápida a nuevos contextos',
          ],
        },
        {
          title: 'Stack técnico',
          items: [
            'Frontend: React, Next.js, TypeScript, SCSS',
            'Backend: Python, Node.js, FastAPI, Express',
            'Data: Oracle SQL, PL/SQL, OCI, PostgreSQL',
            'Herramientas: Docker, Kafka, Git, Vercel',
            'Testing: Vitest, pytest, RTL',
          ],
        },
      ],
    },
    stats: {
      items: [
        { value: '30+', label: 'tecnologías' },
        { value: '6',   label: 'proyectos' },
        { value: '7',   label: 'certificaciones' },
        { value: '2',   label: 'idiomas' },
      ],
    },
    github: {
      label: '// actividad',
      title: 'Actividad en GitHub',
      totalCount: (year) => `{{count}} contribuciones en ${year}`,
    },
    contact: {
      label: '// contacto',
      title: 'Hablemos',
      subtitle: 'Estoy disponible para oportunidades laborales, colaboraciones o simplemente conectar.',
      copy: 'copiar',
      copied: '✓ copiado',
      connection: 'abierto · cifrado · listo',
    },
  },
  en: {
    nav: {
      about: 'About',
      projects: 'Projects',
      education: 'Education',
      skills: 'Skills',
      activity: 'Activity',
      contact: 'Contact',
      openMenu: 'Open menu',
    },
    availability: {
      available_remote: 'Available',
      available_onsite: 'Available',
      not_available: 'Not available',
    },
    hero: {
      title: 'Fullstack & AI Developer',
      description: 'I build SaaS products and AI tools — from prototype to production.',
      cta: 'View projects',
      aboutLabel: '// about me',
      terminal: {
        hint: "type 'help' to explore",
        whoami: ['Deiby Gorrin', 'Fullstack Developer · available for opportunities'],
        help: [
          'help          → show this message',
          'ls            → list sections',
          'goto <section>→ navigate to a section',
          'open github   → open GitHub',
          'open linkedin → open LinkedIn',
          'whoami        → about me',
          'clear         → clear terminal',
        ],
        ls: ['hero', 'about', 'projects', 'education', 'skills', 'activity', 'contact'],
        notFound: (cmd) => `command not found: ${cmd}. Type 'help'.`,
        goto: (section) => `→ navigating to #${section}`,
        gotoUnknown: (section) => `unknown section: ${section}. Use 'ls' to see available ones.`,
        opening: (target) => `→ opening ${target}...`,
      },
      bio1: 'I publish and maintain open source tools for AI agents with real, measurable usage: ClaudeStat (npm) provides cost tracking and loop detection for Claude Code and OpenCode — 2,500+ sessions tracked across 22 projects — and applyr (PyPI), a CLI built natively for AI agents that scores job offers and generates ATS-safe CVs; I used it to analyze 300+ real job offers.',
      bio2: 'Fullstack Developer with completed DAM and Full Stack Master degrees, specializing in Oracle ecosystems. Hands-on experience as Junior Developer at Mercanza, building ERP modules with Kafka, FastAPI, and Docker. In parallel, I build WodRival — a Viking-themed competitive fitness platform built with Next.js and Supabase.',
    },
    projects: {
      label: '// projects',
      title: "What I've built",
      github: 'View on GitHub',
      live: 'Live demo',
    },
    skills: {
      label: '// technologies',
      title: 'Tech stack',
      packagesLoaded: (n) => `${n} packages loaded`,
      levelLabels: { advanced: 'advanced', intermediate: 'intermediate', basic: 'basic', learning: 'learning' },
      sourceLabels: { project: 'project', practice: 'practice', cert: 'certification' },
    },
    education: {
      label: '// education',
      title: 'Education & Certifications',
      certTitle: 'Certifications',
      inProgress: 'In progress',
      coursesLoaded: (n) => `${n} courses loaded`,
      certsVerified: (n) => `${n} certificates verified`,
      verified: '✓ verified',
      viewPdf: 'view PDF',
    },
    certModal: {
      download: 'download',
      closeHint: 'Press ESC · click outside to close',
      openNewTab: 'open in new tab ↗',
    },
    about: {
      label: '// about me',
      terminalFile: 'profile.sh',
      profileCmd: 'cat profile.sh',
      initLine: '> loading profile...',
      nameLine: '> name: "Deiby Gorrin"',
      roleLine: '> role: "Fullstack Developer | AI Developer Tooling"',
      skillsFile: 'skills.sh',
      skillsCmd: 'verify-skills --all',
      status: 'available · open to work',
      checksPassed: (n) => `${n} passed · 0 failed`,
      groups: [
        {
          title: 'Professional profile',
          items: [
            'Analytical thinking',
            'Results-oriented',
            'Immediately available',
            'Completed education (DAM + Full Stack Master)',
            'Quick adaptation to new contexts',
          ],
        },
        {
          title: 'Tech stack',
          items: [
            'Frontend: React, Next.js, TypeScript, SCSS',
            'Backend: Python, Node.js, FastAPI, Express',
            'Data: Oracle SQL, PL/SQL, OCI, PostgreSQL',
            'Tools: Docker, Kafka, Git, Vercel',
            'Testing: Vitest, pytest, RTL',
          ],
        },
      ],
    },
    stats: {
      items: [
        { value: '30+', label: 'technologies' },
        { value: '6',   label: 'projects' },
        { value: '7',   label: 'certifications' },
        { value: '2',   label: 'languages' },
      ],
    },
    github: {
      label: '// activity',
      title: 'GitHub Activity',
      totalCount: (year) => `{{count}} contributions in ${year}`,
    },
    contact: {
      label: '// contact',
      title: "Let's talk",
      subtitle: "I'm available for job opportunities, collaborations, or simply to connect.",
      copy: 'copy',
      copied: '✓ copied',
      connection: 'open · encrypted · ready',
    },
  },
}
