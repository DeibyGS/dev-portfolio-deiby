  ...
  return (
    <section id="education" className="bg-dark-bg py-16 md:py-24 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="space-y-6">
            {education.map((item, index) => (
              <div key={index} className="flex gap-4 items-start">
                <span className="font-mono text-xs text-dark-muted w-36 shrink-0 pt-1 leading-relaxed">
                  {item.period}
                </span>
                <div>
                  <p className="font-sans text-base font-semibold text-dark-text">{item.title}</p>
                  <p className="font-mono text-sm text-dark-muted mt-0.5">{item.institution}</p>
                  {item.inProgress && (
        ...
        {/* Certifications */}
        <FadeIn delay={0.2}>
          <h3 className="font-sans text-base font-semibold text-dark-text mt-12 mb-4">
            {t.certTitle}
          </h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {certifications.map((cert, index) => (
              <button
                ...
                className="flex items-start gap-3 bg-dark-card border border-dark-border hover:border-matrix transition-colors duration-150 p-4 text-left group w-full"
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-sans text-sm font-medium text-dark-text">{cert.name}</p>
                  <p className="font-mono text-xs text-dark-muted mt-0.5">{cert.org} · {cert.year}</p>
                </div>
