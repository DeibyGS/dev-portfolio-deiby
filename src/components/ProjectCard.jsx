function ProjectCard({ project, featured }) {
  ...
  return (
    <article className={`bg-white p-6 flex flex-col gap-4 group hover:bg-[#FAFAFA] transition-all duration-150 ${featured ? 'md:col-span-2' : ''}`}>
      {/* Image area */}
      <div className="aspect-video w-full overflow-hidden border border-cool-gray">
        {imageUrl ? (
          <img
            src={imageUrl}
            alt={name}
      ...
      <div className="flex items-start justify-between gap-4">
        <h3 className="font-mono font-bold text-black text-lg">{name}</h3>
        <div className="flex items-center gap-3 shrink-0">
          <a href={githubUrl} target="_blank" rel="noopener noreferrer"
            aria-label={`Ver ${name} en GitHub`}
            className="text-gray-400 hover:text-matrix transition-colors duration-150">
      ...
      <div className="flex flex-wrap gap-2 mt-auto">
        {stack.map((tech) => (
          <span key={tech}
            className="font-mono text-xs border border-black text-black px-2 py-0.5">
            {tech}
          </span>
        ))}
      </div>
