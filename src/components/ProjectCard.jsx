    ...
    <article className="bg-dark-card p-6 flex flex-col gap-4 group hover:bg-dark-surface transition-all duration-150 h-full">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {githubPrivate && (
            <span className="font-mono text-xs border border-dark-border text-dark-muted px-1">
              [{privateLabel}]
            </span>
          )}
        </div>
        <div className="flex items-center gap-3 shrink-0">
          {githubPrivate ? (
            <span
              aria-label={`${name}: ${privateLabel}`}
              title={privateLabel}
              className="text-dark-muted/40 cursor-not-allowed">
              {githubIcon}
            </span>
          ) : (
      ...
      {/* Evidence badges — verifiable claims (npm version, live metrics, etc.) */}
      {evidence?.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {evidence.map((item) => (
            <span key={item}
              className="font-mono text-xs border border-matrix/40 text-matrix px-2 py-0.5">
              ✓ {item}
            </span>
          ))}
        </div>
