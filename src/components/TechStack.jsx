            ...
            {techs.map(({ label, icon, fallbackText }) => (
              <div key={label} className="flex flex-col items-center gap-2 group cursor-default">
                {icon ? (
                  ...
                  <div className="w-10 h-10 border border-dark-border flex items-center justify-center text-dark-muted text-xs font-mono font-bold group-hover:border-matrix group-hover:text-matrix transition-all duration-150">
                    {fallbackText}
                  </div>
                )}
                <span className="font-mono text-xs text-dark-muted group-hover:text-matrix transition-colors duration-150 text-center leading-tight">
                  {label}
                </span>
              </div>
            ))}
